...
import {
  MessageSquare, Link as LinkIcon, Image as ImageIcon,
  Loader2, Volume2, ShieldAlert, ShieldCheck, AlertTriangle,
  Mic, Lock, Eye, EyeOff, Globe, Calendar, Shield, ExternalLink,
  Zap, Sparkles, Search, CheckCircle2, ArrowRight, BarChart2, Clock
} from 'lucide-react';
...
const TEST_CHIPS = [
  { label: 'SMS Phish', tab: 'text', text: 'URGENT! Your SBI account is blocked due to KYC. Click here immediately: http://bit.ly/sbi-verify-now to avoid suspension.' },
  { label: 'Lottery', tab: 'text', text: 'Congratulations! You have won Rs. 50,000. Pay Rs. 500 registration fee to claim now.' },
  { label: 'Malware Link', tab: 'url', text: 'http://192.168.1.55/login/bank' },
];

const RISK_COLORS = {
  Critical: { bg: 'bg-red-500', text: 'text-red-600', light: 'bg-red-50', border: 'border-red-200', badge: 'bg-red-100 text-red-700' },
  High:     { bg: 'bg-orange-500', text: 'text-orange-600', light: 'bg-orange-50', border: 'border-orange-200', badge: 'bg-orange-100 text-orange-700' },
  Medium:   { bg: 'bg-amber-400', text: 'text-amber-600', light: 'bg-amber-50', border: 'border-amber-200', badge: 'bg-amber-100 text-amber-700' },
  Low:      { bg: 'bg-emerald-500', text: 'text-emerald-600', light: 'bg-emerald-50', border: 'border-emerald-200', badge: 'bg-emerald-100 text-emerald-700' },
};
...
export default function InputForm() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      alert(error.message || 'Analysis failed. Ensure all services are running.');
    } finally {
        ...
        <div className="bg-white rounded-[2rem] border border-slate-200 shadow-sm overflow-hidden p-6 pb-8">
          ...
          <div className="flex gap-2 mb-6">
            {tabs.map((tab) => (
              <button
                key={tab.id}
                onClick={() => { setActiveTab(tab.id as InputType); setAnalysisResult(null); }}
          ...
          <div className="space-y-6">
                  ...
                  <div className="relative group">
                    <div
                      ...
                      className={`w-full h-[180px] border-2 border-dashed rounded-[1.5rem] flex flex-col items-center justify-center cursor-pointer transition-all ${
                        previewUrl ? 'border-slate-200 bg-white' : 'border-slate-200/50 bg-slate-50 hover:bg-slate-100/50'
                      }`}
                    >
                      {previewUrl ? (
                        <>
                          <img src={previewUrl} alt="Preview" className="w-full h-full object-cover p-1 opacity-50 absolute rounded-[1.25rem]" />
                ...
                {activeTab === 'audio' && (
                  <div className="relative group">
                    <div
                      ...
                      className={`w-full h-[180px] border-2 border-dashed rounded-[1.5rem] flex flex-col items-center justify-center cursor-pointer transition-all ${
                        selectedFile ? 'border-slate-200 bg-white' : 'border-slate-200/50 bg-slate-50 hover:bg-slate-100/50'
                      }`}
                    >
                      {selectedFile ? (
                        <div className="text-center z-10">
                          <p className="text-sm font-bold text-slate-600 truncate max-w-[200px] mb-1">{selectedFile.name}</p>
                          <p className="text-xs text-slate-400 font-medium">Ready for transcription</p>
                        </div>
                      ) : (
                        <>
                          <div className="w-10 h-10 bg-white rounded-xl shadow-sm border border-slate-100 flex items-center justify-center mb-2">
            ...
            <div className="flex flex-wrap items-center gap-2 pt-1 border-b border-slate-100 pb-5">
              <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest mr-1">TESTS:</span>
              {TEST_CHIPS.map((chip) => (
                <button
                  key={chip.label}
                  onClick={() => { setActiveTab(chip.tab as InputType); setInputText(chip.text); setAnalysisResult(null); }}
            ...
            <div className="flex flex-col items-center gap-2 pt-2">
              <div className="flex items-center gap-2 text-[11px] text-slate-500 font-medium">
                ...
                <button onClick={() => setShowPrivacyPreview(!showPrivacyPreview)} className="text-blue-600 hover:underline font-bold ml-1">
                  Preview payload 
                </button>
                {showPrivacyPreview ? <EyeOff className="w-3.5 h-3.5 text-blue-600"/> : <Eye className="w-3.5 h-3.5 text-blue-600"/>}
              </div>

              <AnimatePresence>
                {showPrivacyPreview && (
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="w-full overflow-hidden mt-2">
          ...
          {!isAnalyzing && !analysisResult && (
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-4 w-full max-w-xl text-left">
                {[
                  { icon: Zap, title: "Hybrid ML Engine", desc: "Sub-second NLP detection" },
                  { icon: BarChart2, title: "Risk Score Matrix", desc: "Detailed 5-factor breakdown" },
                  { icon: Sparkles, title: "Gemini Recovery", desc: "Context-aware action steps" },
                  { icon: Clock, title: "Legal Timeline", desc: "1-click cybercrime reports" },
                ].map(feature => (
                  <div key={feature.title} className="p-5 rounded-[1.25rem] bg-slate-50 border border-slate-100 flex items-center gap-4">
          ...
          {isAnalyzing && (
            ...
            >
              <div className="relative z-10">
                ...
                <h3 className="text-2xl font-black text-slate-800 mb-3">Analyzing Threat Vectors</h3>
                <p className="text-sm text-slate-500 mb-10 max-w-sm mx-auto">Deep checking against multi-modal models and live threat intelligence feeds...</p>
              </div>
            </motion.div>
          )}
          ...
          {analysisResult && colors && !isAnalyzing && (
            ...
            >
              ...
              <div className={`bg-slate-50 rounded-[2rem] border border-slate-200 shadow-sm overflow-hidden`}>
                <div className={`${colors.bg} px-10 py-8 text-white relative flex flex-col md:flex-row md:items-center justify-between gap-6 overflow-hidden`}>
                  ...
                  <div className="flex items-center gap-6 z-10 w-full">
                    <div className="w-20 h-20 bg-white/20 backdrop-blur-md rounded-[1.5rem] flex items-center justify-center shadow-inner border border-white/30 shrink-0">
                      {isHighRisk ? <ShieldAlert className="w-8 h-8 text-white" /> : <CheckCircle2 className="w-8 h-8 text-white" />}
                    </div>
                    <div className="flex-1">
                      <div className="flex items-center justify-between w-full">
                         <h2 className="text-4xl font-black uppercase tracking-tighter shadow-sm mb-1 leading-none">
                           {isHighRisk ? 'DANGER — SCAM' : analysisResult.risk === 'Low' ? 'SAFE' : 'CAUTION'}
                         </h2>
                         {isHighRisk && (
                      ...
                      <p className="text-white font-medium text-sm pt-2 tracking-wide">
                        {analysisResult.category?.replace(/_/g, ' ')} · AI Confidence Score: <strong>{analysisResult.score}/100</strong>
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-8 pb-10 bg-white">
                  ...
                  <div className={`p-6 rounded-[1.25rem] border ${colors.border} ${colors.light}`}>
                    <p className="text-base md:text-lg text-slate-800 leading-relaxed font-semibold">{analysisResult.recommendation}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-white rounded-[2rem] border border-slate-200 shadow-sm p-8 col-span-1 md:col-span-2">
                   <RiskBreakdown signals={analysisResult.signals} score={analysisResult.score} risk={analysisResult.risk} category={analysisResult.category} />
                </div>
                ...
                {analysisResult.signals?.length > 0 && (
                  <div className="bg-white rounded-[2rem] border border-slate-200 shadow-sm p-8 col-span-1 md:col-span-2">
                    <EvidenceTable signals={analysisResult.signals} category={analysisResult.category} />
                  </div>
                )}
                ...
                {analysisResult.urlMetadata && (
                  <div className="bg-white rounded-[2rem] border border-slate-200 shadow-sm p-8 col-span-1 md:col-span-2">
                    <h3 className="text-sm font-bold text-slate-800 mb-6 flex items-center gap-2">
                      <Globe className="w-5 h-5 text-blue-500" /> Deep Scanner Report
                    </h3>
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                       {analysisResult.urlMetadata.isShortlink && (
                        <div className="col-span-2 lg:col-span-4 p-5 bg-orange-50 border border-orange-200 rounded-[1.25rem]">
                           <p className="text-xs font-bold text-orange-800 mb-2 flex items-center gap-2"><LinkIcon className="w-4 h-4" /> URL Shortener Unmasked:</p>
